...
if (ad_hoc_dims or ad_hoc_metrics or ad_hoc_derived_metrics) and q_name:
    ...
    if st.button('Save Plot (add to Visuals page and YAML)', key='persist_ad_hoc_query'):
        st.session_state.queries.append(q_name)

        # remove all queries except the ones in the session state
        cube.queries = {k:v for k,v in cube.queries.items() if k in st.session_state.queries}
        
        # Save to model catalog (using default nesting behavior)
        ypath = cube.save_to_model_catalog()

        st.success(f"Saved plot '{q_name}' to YAML")
        st.success(f"Saved plot '{q_name}' to Visuals page")
